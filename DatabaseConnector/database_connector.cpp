...
{
	try {
		// Устанавливаем подключение один раз при создании объекта
		conn_ = std::make_shared<pqxx::connection>(db_config_.to_connection_string());
...
pqxx::work database_connector::start_transaction() const
{
	if (!is_connected()) {
...
void database_connector::init_database()
{
	try {
		pqxx::work txn(*conn_);
		if (!table_exists(txn, "documents")) { create_table_documents(txn); }
		if (!table_exists(txn, "words")) { create_table_words(txn); }
		if (!table_exists(txn, "documents_words")) { create_table_documents_words(txn); }
		txn.commit();
	} catch (const std::exception& e) {
...
bool database_connector::table_exists(pqxx::work& txn, const std::string& table_name)
{
...
void database_connector::create_table_documents(pqxx::work& txn)
{
	...
	txn.exec("CREATE INDEX IF NOT EXISTS idx_documents_url_name ON documents(url_name);");
}
...
void database_connector::create_table_documents_words(pqxx::work& txn)
{
	...
	txn.exec("CREATE INDEX IF NOT EXISTS idx_documents_words_url_id ON documents_words(url_id);");
	txn.exec("CREATE INDEX IF NOT EXISTS idx_documents_words_word_id ON documents_words(word_id);");
}
